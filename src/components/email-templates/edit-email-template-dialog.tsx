...
export function EditEmailTemplateDialog({ 
  ...
  const [recipientType, setRecipientType] = useState<'supplier' | 'internal' | 'both'>(template.recipient_type)
  const [tagInput, setTagInput] = useState('')
  const [tags, setTags] = useState<string[]>(template.tags)
  const [stage, setStage] = useState<string | null>(template.stage)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Lỗi khi cập nhật email mẫu:", err)
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="p-8 pb-4 shrink-0">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-slate-500 font-medium text-xs">
              Cập nhật thông tin mẫu email để tối ưu quy trình làm việc.
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-8 pb-8 space-y-6 custom-scrollbar">
          ...
          <DialogFooter className="pt-2 sticky bottom-0 bg-transparent">
            <Button 
              type="submit" 
              disabled={loading} 
              className="w-full rounded-[2rem] h-14 font-black text-base shadow-xl shadow-primary/20 bg-primary text-white hover:bg-primary/90 transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
              {loading ? 'Đang cập nhật...' : 'Cập nhật Email Mẫu'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
